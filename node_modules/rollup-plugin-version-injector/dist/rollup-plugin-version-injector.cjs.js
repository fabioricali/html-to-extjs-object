...
class VIInjector {
    ...
    getVersion(packagePath) {
        const packageFile = JSON.parse(fs__default['default'].readFileSync(path__default['default'].resolve(packagePath), 'utf8'));
    ...
    replaceDate(tag, dateFormat) {
        ...
        let newTag = tag.replace('{date}', dateformat__default['default'](dateFormat));
        if (newTag === tag) {
...
class VILogger {
    ...
    logIt(level, ...args) {
        ...
        if (logLevel >= this.logLevel[1]) {
            this.logger[level](chalk__default['default'].magenta.bold('  [VI]'), chalk__default['default'].yellow(`[${level.padEnd(5)}]`), ...args);
        }
